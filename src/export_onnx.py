...
ONNX_OUTPUT_PATH = OUTPUT_DIR / f"{RUN_ID}_decoder.onnx"
SCALER_OUTPUT_PATH = OUTPUT_DIR / f"{RUN_ID}_scaler.pkl"
...
try:
    ...
    decoder_outputs = vae.decoder(latent_input, training=False)

    # 3. Wrap in a static Functional Keras Model
    export_model = tf.keras.Model(  # type: ignore
        inputs=latent_input, outputs=decoder_outputs, name="onnx_decoder"
    )
